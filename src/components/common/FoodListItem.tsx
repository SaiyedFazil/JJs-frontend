...
export const FoodListItem = memo(
  ({ item, onAdd, onRemove, quantity = 0 }: FoodListItemProps) => {
    return (
      <View className="py-6 flex-row items-start border-b border-border/20 dark:border-border-dark/20">
        ...
        <View className="flex-1 pr-4">
          ...
          <View
            className={`w-4 h-4 border-2 items-center justify-center rounded-sm mb-2 ${item.isVeg ? 'border-green-600' : 'border-red-600'}`}
          >
            <View
          ...
          <Text
            className="text-foreground dark:text-foreground-dark text-[17px] font-black leading-6 mb-1"
            numberOfLines={2}
          >
            {item.name}
          </Text>
          ...
          <Text
            className="text-muted dark:text-muted-dark text-[13px] leading-5 font-medium"
            numberOfLines={2}
          >
            {item.description ||
              'Fresh and delicious meal prepared with the finest ingredients.'}
          </Text>
        </View>
            ...
            <View className="absolute -bottom-3 left-0 right-0 items-center">
              {quantity > 0 ? (
                <View className="flex-row items-center bg-primary dark:bg-primary-dark rounded-xl shadow-lg px-2 py-2">
                  ...
                  <Text className="mx-4 text-white font-black text-base">
                    {quantity}
                  </Text>
                ...
                >
                  <Text className="text-primary-foreground dark:text-primary-foreground font-black text-sm uppercase tracking-wider">
                    ADD
                  </Text>
                </TouchableOpacity>
              )}
